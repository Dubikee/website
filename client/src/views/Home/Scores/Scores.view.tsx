...
class ScoresView extends React.Component<IScoresViewPorps>{
	...
	render() {
		...
		return <div className="scores-views">
			<Tabs defaultActiveKey="2">
				...
				<Tabs.TabPane tab="考试成绩" key="2">
					<div className="cp-controller">
						...
						<Select defaultValue={year} style={{ minWidth: 140, marginRight: 20 }} onChange={p => this.setState({ year: p })}>
					...
					<div className="ph-controller">
						<Select size="small" defaultValue={year} style={{ minWidth: 120, marginRight: 5 }} onChange={p => this.setState({ year: p })}>
							<Select.Option value="*">全部</Select.Option>
							{Array.from(yearSet).map((y, i) => <Select.Option key={i} value={y}>{y}</Select.Option>)}
						</Select>
						<span style={{ fontSize: 14 }}>学年</span>
						<Button size="small" style={{ float: 'right' }} type="dashed" onClick={() => this.refresh()} icon='reload'>刷新</Button>
					</div>
